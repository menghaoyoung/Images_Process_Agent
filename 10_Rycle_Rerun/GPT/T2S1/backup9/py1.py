...
# Set default image and output paths
...
def main():
    ...
    # Get line points and calculate length in mm
    line_points = get_line_points(start_point, end_point)
    num_pixels = len(line_points)
    pixel_distance = np.hypot(end_point[0] - start_point[0], end_point[1] - start_point[1])
    length_mm = pixel_distance * resolution
    print(f"Line from {start_point} to {end_point}: {num_pixels} pixels, length = {length_mm:.2f} mm (resolution = {resolution} mm/pixel)")
