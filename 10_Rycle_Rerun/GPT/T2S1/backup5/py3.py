...
if __name__ == "__main__":
    # Check if run from command line with -resolution argument
    if len(sys.argv) == 1:
        # No arguments given: prompt user for resolution interactively
        try:
            resolution = float(input("Please input the image resolution (physical unit per pixel): ").strip())
        except Exception:
            print("Invalid input for resolution. Exiting.")
            sys.exit(1)
        main(resolution)
    else:
