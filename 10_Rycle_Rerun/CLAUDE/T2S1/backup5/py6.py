...
def get_line_grayscale(image_path, start_point, end_point, resolution):
    ...
    return grayscale_values, distance_mm, output_file
...
def process_image(resolution):
    ...
    grayscale_values, distance_mm, csv_file = get_line_grayscale(image_path, start_point, end_point, resolution)
    ...
    if grayscale_values is not None:
        ...
        # Save plot
        output_dir = r"C:\Users\admin\Desktop\Python_proj\ALL_RESULT\CLAUDE\T2S1\backup5"
        plt.savefig(os.path.join(output_dir, f"grayscale_plot_res{resolution}.png"))
        print(f"Plot saved to {output_dir}")
    
    return csv_file

def verify_results(resolution):
    output_dir = r"C:\Users\admin\Desktop\Python_proj\ALL_RESULT\CLAUDE\T2S1\backup5"
    csv_file = os.path.join(output_dir, f"grayscale_values_res{resolution}.csv")
...
def main():
    # Fixed resolution value - no command line arguments needed
    resolution = 1.08
    
    # Process the image
    print(f"Processing image with resolution={resolution}...")
    csv_file = process_image(resolution)
    
    # Verify the results
    verify_results(resolution)
