# py1.py - Calculate grayscale values along a line and verify results
import os
import numpy as np
...
# Function to save grayscale data to CSV
def save_grayscale_data(grayscale_data, resolution, output_dir):
    # Create output filename
    output_filename = f"line_grayscale_res_{resolution}.csv"
...
def create_plot(grayscale_data, line_length, resolution, output_dir):
    ...
    # Save the plot
    plot_filename = f"line_grayscale_plot_res_{resolution}.png"
...
def main():
    # Define parameters - NO COMMAND LINE ARGUMENTS
    resolution = 1.08  # Fixed resolution value
    ...
    grayscale_data, line_length = get_line_grayscale(image_path, start_point, end_point, resolution)
    
    if grayscale_data is not None:
        # Save data to CSV
        csv_path = save_grayscale_data(grayscale_data, resolution, output_dir)
        
        # Create and save plot
        create_plot(grayscale_data, line_length, resolution, output_dir)
        ...
        print("Failed to calculate grayscale values")
