...
def check_gap_conditions(img_array, row, col, threshold_min=1, threshold_max=150, contiguous_count=25):
    ...
    for dr, dc in directions:
        ...
        if contiguous >= contiguous_count:
...
def process_images(input_directory):
    ...
    enhanced_dir = os.path.join(os.path.dirname(input_directory), "ALL_RESULT", "CLAUDE", "T3", "backup8", "enhanced")
    os.makedirs(enhanced_dir, exist_ok=True)
    
    # Create output directory for result images
    output_dir = os.path.join(os.path.dirname(input_directory), "ALL_RESULT", "CLAUDE", "T3", "backup8")
