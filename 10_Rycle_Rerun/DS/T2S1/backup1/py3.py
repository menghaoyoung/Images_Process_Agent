...
def main():
    # Set up robust argument parsing
    ...
    resolution_str = args.resolution.strip()
    try:
        resolution_value = float(resolution_str)
    except ValueError:
        sys.exit(f"Error: Invalid resolution value '{args.resolution}'. Must be a number.")
    ...
    print(f"Success: Grayscale values saved to {output_file}")
