...
def main():
    ...
    if len(sys.argv) != 2 or not sys.argv[1].startswith('-resolution='):
        sys.exit("Usage: python py1.py -resolution=<value> (e.g. -resolution=1.08)")
    
    # Extract resolution value
    try:
        resolution_str = sys.argv[1].split('=', 1)[1]
        resolution_value = float(resolution_str)
    except (IndexError, ValueError):
        sys.exit("Error: Invalid resolution format. Use -resolution=1.08")
    ...
    print(f"Success: Saved {len(grayscale_values)} points to {output_file}")
